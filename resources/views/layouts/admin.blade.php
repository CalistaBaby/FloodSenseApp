...
<head>
    ...
    <title>FloodSense | Admin BPBD</title>
...
<body class="bg-[#F0F7FF] font-sans antialiased text-slate-900">
    ...
    <aside class="fixed left-0 top-0 h-screen w-64 bg-[#144cce] border-r border-white/10 z-50 flex flex-col shadow-2xl">
        ...
        <nav class="flex-1 p-4 space-y-2 mt-4">
            <a href="/admin/dashboard" 
               class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->is('admin/dashboard') ? 'bg-white/20 text-white' : 'text-blue-100 hover:bg-white/10' }}">
            ...
            <a href="/admin/laporan/create" 
               class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->is('admin/laporan/create') ? 'bg-white/20 text-white' : 'text-blue-100 hover:bg-white/10' }}">
                <i class="fas fa-bullhorn text-lg w-6 text-center"></i>
                <span class="text-xs font-black uppercase tracking-widest">Laporan Resmi</span>
            </a>

            <a href="/admin/laporan/validasi" 
               class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->is('admin/laporan/validasi') ? 'bg-white/20 text-white' : 'text-blue-100 hover:bg-white/10' }}">
                <i class="fas fa-clipboard-check text-lg w-6 text-center"></i>
                <span class="text-xs font-black uppercase tracking-widest">Laporan Umum</span>
            </a>
        </nav>
        ...
        <header class="h-20 bg-white/90 backdrop-blur-md sticky top-0 z-40 border-b border-blue-100 px-10 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right hidden sm:block">
                    <p class="text-[10px] font-black text-slate-400 uppercase leading-none mb-1 text-right">Administrator</p>
                    <p class="text-sm font-bold text-blue-900 leading-none text-right">{{ auth()->user()->name }}</p>
                </div>
